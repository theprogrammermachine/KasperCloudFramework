...
{
    public class KafkaGatewayExtension : KafkaExtension
    {
        public static void SetupGatewayConsumer<T, V>(IEnumerable<Dictionary<string, string>> configs, Bugsnag.Client bugsnag, Action<object> signalRPipe) where V : Answerable
        {
                            ...
                            while (true)
                            {
                                try
                                {
                                    ...
                                    switch (cr.Value.MessageType)
                                    {
                                        ...
                                        case MessageWrapper<object>.WrapperType.OutNetNotif:
                                        {
                                            signalRPipe(cr.Value.Message);
                                            break;
                                        }
                                    }
                                }
        ...
        public static void SetupGatewayConsumer<T, V>(IEnumerable<Dictionary<string, string>> configs, Action<object> signalRPipe) where V : Answerable
        {
            SetupGatewayConsumer<T, V>(configs, null, signalRPipe);
        }
    }
}